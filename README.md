...
RAG-On-Obsidian — это инструмент, который позволяет взаимодействовать со своими заметками в Obsidian через Retrieval-Augmented Generation (RAG). Приложение предлагает автоматическую индексацию, эффективный поиск и визуализацию данных для глубокого анализа и понимания вашей базы знаний.
...
1. **Frontend**:
   ...
   - Web-интерфейс, разработанный с использованием TypeScript и Vite.
   - Обеспечивает удобный интерфейс для взаимодействия пользователя с системой.

2. **Backend**:

   - Основная серверная часть, реализованная с использованием FastAPI и SQLAlchemy.
   - Обрабатывает пользовательские запросы и взаимодействует с векторной базой данных.

3. **Векторная база данных**:

   - Используются Qdrant и PGVector для хранения векторных представлений документов и текстовых метаданных.
   - Обеспечивает быстрый и точный поиск релевантных данных.

4. **RAG Пайплайн**:

   - Включает в себя обработку текста, генерацию эмбеддингов и поиск релевантных данных.
   - Работает на основе LangChain и sentence_transformers.

5. **Система визуализации**:

   - Построена на основе PCA-разложения для отображения взаимосвязей между заметками в формате облака точек.
   - Позволяет пользователям анализировать структуру их базы знаний.
...
Если у вас есть вопросы или предложения, свяжитесь с разработчиками проекта:
